...
app.get('/consultRequest/:id', function(req,res){
  ...
  db.request.findOne({_id: id}, function(err,doc){
    res.json(doc);
  });
});

app.get('/consultRequest/:who', function(req,res){
  db.request.findOne({fullName: who}, function(err,doc){
    res.json(doc);
  });
});
...
app.get('/consultFullDaysByLab/:lab', function(req,res){
  ...
  db.laboratory.findOne({name:lab}, function(err,doc){
    res.json(doc);
    console.log(doc);
  });
});
...
app.get('/consultAdmin/:who', function(req,res){
  ...
  db.administrator.findOne({name:who}, function(err,doc){
    res.json(doc);
    //console.log(doc);
  });
});
...
app.get('/consultRequestsInSpecTime/:day',function(req,res){
	...
	db.request.find({availability:{$in:[day]}},
		function(err,doc){
			//console.log(doc);
			res.json(doc);
		});
...
app.delete('/delRequest/:id', function(req,res){
  var id = mongojs.ObjectId(req.params.id);
  db.request.remove({_id: id}, function(err,doc){
    res.json(doc);
  });
});
...
app.delete('/delLab/:id', function(req,res){
  var id = mongojs.ObjectId(req.params.id);
  db.laboratory.remove({_id: id}, function(err,doc){
    res.json(doc);
  });
});
...
app.delete('/delAdministrator/:id', function(req,res){
  var id = mongojs.ObjectId(req.params.id);
  db.administrator.remove({_id: id}, function(err,doc){
    res.json(doc);
  });
});
...
app.put('/updateAdministrator/:id', function(req,res){
  var id = mongojs.ObjectId(req.params.id);
  db.request.findAndModify({query: {_id: id}, 
    update: 
      {$set: 
        {
          name: req.body.name,
...
var id = mongojs.ObjectId(req.params.lab);
  db.laboratory.findAndModify({query: {_id: id}, 
    update: 
      {$set: 
        {          
          l1: req.body.selL1,
